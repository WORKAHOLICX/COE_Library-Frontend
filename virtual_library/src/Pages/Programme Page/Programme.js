      ...
      <VideoContainer>
        <VideoBg>
          <Video1
            ...
            loop
            muted
            src={apro.length !== 0 ? apro[0].vid : ""}
            type="video/mp4"
          />
        </VideoBg>
        <VideoContent>
          {apro.length !== 0 ? (
            <VideoHeader>{apro[0].name.toUpperCase()}</VideoHeader>
          ) : (
            ""
          )}
        </VideoContent>
          ...
          <InfoRow>
            <Column1>
              {apro.length !== 0 ? (
                <h1>
                  {" "}
                  <ProgrammeInfo>{apro[0].paragraph}</ProgrammeInfo>
                </h1>
              ) : (
                ""
              )}
      ...
      <div className="Everything" style={{ display: "flex" }}>
        ...
        justify-content-between "
          ...
          >
            <div>
              <h2>
                {Number(courseQuery) === 1
                  ? "LEVEL 100"
                  : Number(courseQuery) === 2
                  ? "LEVEL 200"
                  : Number(courseQuery) === 3
                  ? "LEVEL 300"
                  : Number(courseQuery) === 4
                  ? "LEVEL 400"
                  : ""}
              </h2>{" "}
            </div>
          ...
          <CourseCardContainer>
            <CourseCardWrapper>
              {cour
                .filter((course) => {
                  if (
                    Number(course.year) === Number(courseQuery) &&
                    Number(course.semester === 1)
                  )
                    return course;
                  return "";
                })
